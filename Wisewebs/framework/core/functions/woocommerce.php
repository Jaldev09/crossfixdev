...
if ( class_exists( 'WooCommerce' ) ) {
    ...
    if ( ! function_exists('yolo_woocommerce_sale_flash') ) {
        function yolo_woocommerce_sale_flash($sale_flash,$post,$product) {
            ...
            if ($product_sale_flash_mode == 'percent') {
                ...
                if ($sale_percent > 0) {
                    return '<span class="on-sale product-flash">' . $sale_percent . '% ' . esc_html__( 'Sale', 'woocommerce' ) . '</span>';
                } else {
                    return "";
                }
    ...
    add_action('woocommerce_after_shop_loop_item_title','woocommerce_template_loop_product_title',1);
    ...
    function yolo_woocommerce_wishlist(){
        ...
        <a title="<?php esc_html_e( 'Wishlist','yolo-motor' ); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="yolo-wishlist">
            ...
            <?php echo '<span class="total">' . ($count > 0 ? zeroise($count, 2) : '0') . '</span>'; ?>
        </a>
...
<?php
    ...
    if ( ! function_exists( 'yolo_paging_nav_product' ) ) {
        function yolo_paging_nav_product($products) {
            ...
            $page_links = paginate_links( array(
                ...
                'prev_text' => esc_html__('Prev', 'woocommerce'),
    ...
    function yolo_category_menu() {
       ...
       global $wp_query;
       $cat_all = 1;
       $current_tax = $wp_query->taxonomy;
       if(isset($current_tax) && $current_tax !=''):
       ...
       if ( $cat_all ) {
           $output .= '<li class="current-cat"><a href="' . home_url() . '/shop/">' . esc_html__( 'All', 'yolo-motor') . '</a></li>'; // @TODO: change shop to get shop page
       } else {
           $output .= '<li class=""><a href="' . home_url() . '/shop/">' . esc_html__( 'All', 'yolo-motor') . '</a></li>';
       }
       if( $categories ){
           foreach ( $categories as $key => $val ) {
               if ( $val->count > 0 ) {
