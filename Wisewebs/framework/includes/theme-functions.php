...
if (!function_exists('yolo_custom_css_variable_font')) {
	function yolo_custom_css_variable_font() {
		...
		// General Font
		$fonts->primary_font_family = 'Varela Round';
...
if (!function_exists('yolo_custom_css_variable_logo')) {
	function yolo_custom_css_variable_logo($page_id = 0) {
		...
		$logo_matrix = array(
			...
			'header-2' => array(140, 20, 20, 80),
			'header-3' => array(140, 20, 20, 80),
		...
		if (!empty($page_id)) {
			...
			if (isset($yolo_options['logo_padding']) && is_array($yolo_options['logo_padding'])) {
				if (isset($yolo_options['logo_padding']['padding-top']) && !empty($yolo_options['logo_padding']['padding-top'])) {
					$logo->logo_padding_top = $yolo_options['logo_padding']['padding-top'];
				}
				if (isset($yolo_options['logo_padding']['padding-bottom']) && !empty($yolo_options['logo_padding']['padding-bottom'])) {
					$logo->logo_padding_bottom = $yolo_options['logo_padding']['padding-bottom'];
				}
			}
		}
...
if (!function_exists('yolo_custom_css_variable_header')) {
	function yolo_custom_css_variable_header($page_id = 0) {
		...
		$enable_page_color = (!empty($page_id) && ($page_id !== 0)) ? get_post_meta($page_id,$prefix . 'enable_page_color', true) : '0';
		if ($enable_page_color == '1') {
			$primary_color = (!empty($page_id) && ($page_id !== 0)) ? get_post_meta($page_id,$prefix . 'primary_color', true) : '';
		}
		else {
...
if (!function_exists('yolo_custom_css_variable')) {
	function yolo_custom_css_variable($page_id = 0) {
		...
		$primary_color      = '#ffb535';
		$link_color         = '#939393';
		$link_color_hover   = '#ffb535';
		$link_color_active  = '#ffb535';
		$enable_page_color = (!empty($page_id) && ($page_id !== 0)) ? get_post_meta($page_id,$prefix . 'enable_page_color',true) : '0';
		
		if ($enable_page_color == '1') { // Use metaboxes settings
			$primary_color            = get_post_meta($page_id,$prefix . 'primary_color', true);
			$link_color               = get_post_meta($page_id,$prefix . 'link_color',true);
			$link_color_hover         = get_post_meta($page_id,$prefix . 'link_color_hover', true);
			$link_color_active        = get_post_meta($page_id,$prefix . 'link_color_active', true);
			...
			$top_bar_bg_color         = yolo_hex2rgba( $top_bar_bg_color, $top_bar_bg_color_opacity );
		}
			...
			if (isset($yolo_options['link_color']) && ! empty($yolo_options['link_color']) && !empty($yolo_options['link_color']['regular'])) {
				$link_color = $yolo_options['link_color']['regular'];
			}
			if (isset($yolo_options['link_color']) && ! empty($yolo_options['link_color']) && !empty($yolo_options['link_color']['hover'])) {
				$link_color_hover =  $yolo_options['link_color']['hover'];
			}
			if (isset($yolo_options['link_color']) && ! empty($yolo_options['link_color']) && !empty($yolo_options['link_color']['active'])) {
				$link_color_active = $yolo_options['link_color']['active'];
			}
		}
