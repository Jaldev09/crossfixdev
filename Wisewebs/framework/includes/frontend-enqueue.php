...
if (!function_exists('yolo_enqueue_styles')) {
	function yolo_enqueue_styles() {
		...
		if ( !(defined('YOLO_SCRIPT_DEBUG') && YOLO_SCRIPT_DEBUG) ) {
			wp_enqueue_style('yolo-framework-style', get_template_directory_uri() . '/style'.$min_suffix.'.css');
		}
		/* VC Customize */ 
		...
		if (is_rtl() || $enable_rtl_mode == '1' || isset($_GET['RTL'])) {
			wp_enqueue_style('yolo-framework-rtl', get_template_directory_uri() . '/assets/css/rtl'.$min_suffix.'.css');
		}
	}
...
if (!function_exists('yolo_enqueue_script')) {
	function yolo_enqueue_script() {
		...
		$translation_array = array(
			'product_compare'        => esc_html__( 'Compare', 'woocommerce' ),
			'product_wishList'       => esc_html__( 'WishList', 'woocommerce' ),
			'product_wishList_added' => esc_html__( 'Browse WishList', 'woocommerce' ),
			'product_quickview'      => 'Snabbtitt',
			'product_addtocart'      => esc_html__( 'Add To Cart', 'woocommerce' )
		);
