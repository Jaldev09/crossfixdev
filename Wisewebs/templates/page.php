...
if (($sidebar === '') || ($sidebar == '-1')) {
	$sidebar = $yolo_options['page_sidebar'] ? $yolo_options['page_sidebar'] :'right';
}

$left_sidebar = get_post_meta(get_the_ID(),$prefix.'page_left_sidebar',true);
if (($left_sidebar === '') || ($left_sidebar == '-1')) {
	$left_sidebar = $yolo_options['page_left_sidebar'] ? $yolo_options['page_left_sidebar'] : 'sidebar-1';

}

$right_sidebar = get_post_meta(get_the_ID(),$prefix.'page_right_sidebar',true);
if (($right_sidebar === '') || ($right_sidebar == '-1')) {
	$right_sidebar = $yolo_options['page_right_sidebar']? $yolo_options['page_right_sidebar'] : 'sidebar-1';
}

$sidebar_width = get_post_meta(get_the_ID(),$prefix.'sidebar_width',true);
if (($sidebar_width === '') || ($sidebar_width == '-1')) {
	$sidebar_width = $yolo_options['page_sidebar_width']? $yolo_options['page_sidebar_width']: 'small';
}
