 ...
 */
class LargeHeroImage extends Content {

	// Class constants
	const FIELD_HEADING         = 'heading';
	const FIELD_IMAGE           = 'large_hero_image';
	const FIELD_TONE_DOWN_IMAGE = 'tone_down_image';

	// Image sizes
	const IMAGE_SIZE_NAME        = self::FIELD_IMAGE;
	const IMAGE_SIZE_WIDTH       = 1650;
	const IMAGE_SIZE_HEIGHT      = 400;

	// CSS classes
	const CSS_CLASS_SECTION      = 'large-hero-image';
	...
	protected static function content( $data = Array() ) {
		...
		$defaults = Array(
			...
			'ingress'             => '',
			self::FIELD_HEADING   => '',
			self::FIELD_TONE_DOWN_IMAGE => false,
		);
...
?>
		<div class="yolo-page-title-section page-title-margin">
			<section class="yolo-page-title-wrap page-title-height page-title-wrap-bg page-title-center" style="background-image: url(<?php echo $data[ self::FIELD_IMAGE ][ 'url' ]; ?>);">
				<div class="yolo-page-title-overlay <?php if( true === ( bool ) $data[ self::FIELD_TONE_DOWN_IMAGE ] ) { ?>toned-down<?php } ?>"></div>
				<div class="container">
					<div class="page-title-inner block-center">
						<div class="block-center-inner">
							<h1 class="heading"><?php echo $data[ self::FIELD_HEADING ]; ?></h1>
							<div class="ingress">
<?php
								echo $data[ 'ingress' ];
?>
